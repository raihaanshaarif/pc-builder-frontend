    ...
    <>
      <div className="container mx-auto mt-24">
            ...
            <div className="mx-4 grid grid-cols-2 gap-4 md:grid-cols-4">
                {rams && rams.length > 0 ? rams.map((product) => (
...
export default RamrPage;

export async function getStaticProps() {
    const baseURL = process.env.NEXT_PUBLIC_API_URL;
  try {
      const res = await fetch(`${baseURL}/api/v1/products/?category=ram`);
      ...
      return {
          props: { rams: data.data }, // ensure you pass the correct part of the response
      };
  } catch (error) {
      console.error("Error fetching product data:", error);
      return {
          props: { rams: [] }, // return empty array or appropriate fallback
      };
  }
}
